...
\subsection{Generalized Structured Additive Regression}
...
Generalized Linear Models (GLM), first coined by \citet{nelder1972}, introduce a framework where the response $y$ is related to a linear predictor $\textbf{x}'\boldsymbol{\beta}$ via a link function $\eta = g(E(y))$ to arrive at the following model specification:
% y = ...
% eta = xB
 % The main advantage is that now multiple different responses are supported
% weiter GLM explainen, dann sagen wie : link function extends STAR models zu generalized Star models

\subsection{Bayesian Additive Models for Location, Scale and Shape}

% howevery only possible for exponential families
