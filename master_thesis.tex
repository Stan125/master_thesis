...
\end{equation*}
 can described in matrix notation to extend \eqref{addmatform} in the following way:
\begin{equation*}
...
\end{equation}
where $\bbeta_{jl}$ depicts regression coefficients for the $j$th predictor on distributional parameter $\theta_l$ and therefore serves as a subset of $\bbeta$, and $\by, \bx$ are response and explanatory variables respectively. Vectors $\btau$ and $\balpha$ are both parameters of the prior distributions $p_{jl}(\cdot)$, whereas $\btau$ represents hyper-parameters with their own distribution $d_{\btau_{jl}(\cdot)}$, while $\balpha$ is fixed and not varying. Thus, the prior distributions in the bamlss framework can be further dissected into
\begin{equation}
  \label{prior}
...
\begin{equation*}
  \alpha(\bbeta_{jl}^{*} | \bbeta^{(t)}_{jl}) = min [\frac{\pi(\bbeta_{jl}^{*} | \cdot)}{\pi(\bbeta_{jl}^{(t)} | \cdot)}, 1]
\end{equation*}
where the log-posterior density is evaluated at both the current and proposed value. In common cases, the jumping distribution is denoted by a normal distribution with expectation $\bbeta_{jl}^{(t)}$ and covariance matrix $\Sigma_{jl})$. For multivariate priors nonlinear effects, \citet{bamlss2017} suggest to set the covariance matrix to $\Sigma_{jl} = \sigma_{jl} \mathbf{P}_{jl}(\btau_{jl})^{-1}$, where $\mathbf{P}_{jl}(\btau_{jl})$ depicts the precision matrix for smoothing variance $\btau_{jl}$. \par
Furthermore, the authors suggest the use of a \enquote{Derivative-based Metropolis-Hastings} sampler, where the jumping distribution is again a normal distribution, but its expectation and covariance matrix are iteratively recalculated using iterative weighted least squares, where at each step a new working variable $\mathbf{z}_l$ and weights $\mathbf{W}_{ll}$ depending on current evaluations of the log-likelihood function $l(\bbeta | \by, \bx)$ are defined \citep{bamlss2017}. \par
% Hier noch slice sampling erklären

\section{bamlss.vis}
\label{bamlss.vis}
The previous Sections \ref{AM} to \ref{estimation} gave a description of Bayesian Additive Models for Location, Scale and Shape (bamlss) and the underlying sub-models on which they are based. This section will introduce a framework to interactively visualize covariate effects and distributional predictions of fitted bamlss models and feature its implementation as an R package. Because of the visual component, the tool will be called bamlss.vis. A small case-study based on wages of male workers in the Mid-Atlantic region of the United States of America will be presented to feature most of bamlss.vis' abilities. \par
...
\label{case_study}
While automatic testing of bamlss.vis' main functions relies on artificial data for each supported distribution in order to prove correct behavior, presenting the apps' abilities is best done with a dataset of real observations. This chapter will focus on fitting a bamlss based on \enquote{real} data for further use in bamlss.vis. The objective for a suitable dataset was that its response variable and explanatory variables are easy to understand for people without a specific scientific background. The chosen dataset, \enquote{Wage} from the ISLR R package \citep{ISLR}, perfectly fulfills these requirements. \enquote{Wage}, collected by the \citet{uscensus}, includes 3000 male individuals with records of the following variables:
\begin{itemize}
...
\label{scenarios_tab}
After selecting a bamlss object, one can then click on the \enquote{Scenarios} tab. In this tab, the user can create one or more \enquote{Scenarios} by interactively specifying covariate values for each explanatory variable. After these combinations are \enquote{locked in} by clicking a button, the predicted distribution based on the previously selected model is then plotted. \par
...
Under the visual components of the \enquote{Scenarios} rests a small database, which stores each covariate combination the user has specified and is cleared when the user clicks on \enquote{Clear Scenarios}. This database, created with \li{shiny::reactiveValues()} forms the basis for all other tabs which analyse those covariate combinations. It has a \enquote{reactive} nature, such that all tabs depending on it are automatically updated when a database value changes. \par
...
The third web element on the right side of the plot output, a red button with the description \enquote{Obtain Code!}, adds user reproducibility to the plot. When clicked, a modal window pops up with R commands that, if executed in the main R console with the users current working environment, directly recreates the graph in the \enquote{Plot} tab. Figure \ref{obtain_code} shows the modal window which arises when pressing the \enquote{Obtain Code!} button in the interface of Figure \ref{educ_app}.
\begin{figure}
  \begin{centering}
...
To edit an observation from categorical variables, users can click on a small drop-down button in the cell which is to be edited. The table recognizes categorical variables and will then provide a menu where the desired value is to be selected. With numeric variables, users can select the cell and then input the value they wish to make predictions with. Values in logical variables can be specified by checking or unchecking a box in the cell.\par
