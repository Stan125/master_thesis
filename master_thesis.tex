...
\end{equation}
In Equation \eqref{gamlss}, $g_l(\cdot)$ represents a known monotonic link function, which can be different for each parameter. $\bx_l$ depicts the subset of variables $\bx = (\mathbf{x}_1, \ldots, \mathbf{x}_Q)'$ used to model parameter $\theta_l$, while $\mathbf{Z}_{jl} \bgamma_{jl}$ serves as the Basis function matrix for a non-parametric effect of covariate $z_j$ on parameter $\theta_l$, taken from a subset of variables $z_1, \ldots, z_K$ variables. The specific subset of covariates $z$ with non-parametric effects on parameter $\theta_l$ has a length of $K_l$ variables \citep{gamlss2007}. \par
...
\begin{figure}[ht]
  \begin{centering}
    \includegraphics[scale = 0.2, trim = {7.5cm 4cm 5cm 2cm}]{images/035_educ_app_cdf.png}
