...
This problem can be well illustrated using the censored normal distribution. Assume a normally distributed variable, $y \sim N(\mu, \sigma^2)$. Then, the probability density function of a left-censored normal distribution $y^*$ with cut-off point $a = 0$ can be obtained by
\begin{equation*}
  f(y^* = x) =
  \begin{cases}
    f(y = x) & x > 0 \\
  ...
  \end{cases}
\end{equation*}
where $f(y)$ and $F(y)$ are the probability density functions and the cumulative distribution function of normally distributed variable $y$, respectively. It is visible in the above equation that the censored normal distribution is both discrete and continuous. While $y^*$ shares the density with $y$ above the cut-off point, the full remaining density in the censored normal distribution $y^*$ created from $y \sim N(0, 1)$ with cut-off point $a = 0$ is assigned to the cut-off point. Figure \ref{cnorm} shows a sample left-censored normal distribution.  \citep{greene2012}.

\begin{figure}
  \begin{centering}
    \includegraphics[scale = 0.5]{images/06_cnorm_plot.pdf}
  \caption{Probability Density Function of a left-censored normal distribution.}
  \label{cnorm}
  \end{centering}
\end{figure}
